...
import { privateExams } from '../data/examData';
...
export const Private: React.FC = () => {
  ...
  const departments = [...new Set(privateExams.map(exam => exam.department))];
  const difficulties = ['Beginner', 'Intermediate', 'Advanced'];

  const filteredExams = privateExams.filter(exam => {
  ...
  const stats = [
    { label: 'Total Exams', value: privateExams.length, icon: BookOpen },
    { label: 'Total Posts', value: privateExams.reduce((sum, exam) => sum + exam.posts, 0).toLocaleString(), icon: Users },
    { label: 'Upcoming Exams', value: privateExams.filter(exam => exam.importantDates.some(date => date.status === 'upcoming')).length, icon: Calendar }
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
              Prepare for top private companies and multinational corporations. 
              Master technical interviews, aptitude tests, and skill assessments for your dream job.
            </p>
          </div>
          ...
          <>
            <div className="mb-6">
              <p className="text-gray-600 dark:text-gray-400">
                Showing {filteredExams.length} opportunity{filteredExams.length !== 1 ? 'ies' : 'y'}
